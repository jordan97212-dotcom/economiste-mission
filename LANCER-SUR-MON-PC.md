...
| Réparer l'historique des migrations (erreur P3005) | `Reparer-base.bat` |
...
| Le journal dit `P3005` — « The database schema is not empty » | Votre base contient des tables, mais Prisma n'y trouve pas la trace de ce qui a déjà été appliqué, et refuse d'y toucher. Lancez `Reparer-base.bat` : il regarde la base et reconstitue cette trace, sans toucher à vos données. Puis `Demarrer.bat`. |
| Le journal parle de `Query Engine` introuvable | Le moteur de base de données ne correspond pas au système de l'image. L'application embarque désormais les deux variantes ; si le message persiste, lancez `Reconstruire.bat`. |
